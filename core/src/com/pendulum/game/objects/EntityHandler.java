...
public class EntityHandler {
    ...
    public void createRope(Vector3 position, World world) {
        joint = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createCollisionBody(new Vector2(position.x / PPM,
                        position.y / PPM),
                JOINT_RADIUS, world, BodyDef.BodyType.StaticBody, CATEGORY_JOINTS, MASK_JOINTS), textureHolder.getTexture("ball_" + collectibles.getBallName()), new Vector2(JOINT_RADIUS, JOINT_RADIUS), "joint");
    ...
    public void setupStartScreen(float topy, World world) {
        joint_initial = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createCollisionBody(new Vector2(WORLD_WIDTH/PPM/2, (topy/PPM) + REAL_WORLD_HEIGHT/PPM/2),
                PLAYER_RADIUS, world, BodyDef.BodyType.StaticBody, CATEGORY_PLAYER, MASK_PLAYER), textureHolder.getTexture("ball_" + collectibles.getBallName()), new Vector2(JOINT_RADIUS, JOINT_RADIUS), "joint");
        ...
        this.rod = com.pendulum.game.objects.ObjectHandler.createRope(joint_initial.getBody(), player.getBody(), world, "ROD", textureHolder, 0.0f, collectibles.getRodName()).get(0);
