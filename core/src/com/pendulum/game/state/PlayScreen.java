...
public class PlayScreen {
    ...
    private final float color_timeperiod = 0.3f;
    ...
    private float collectiblescreens = 0.0f;
    ...
    public PlayScreen(com.pendulum.game.MyGdxGame app) {
        ...
        REAL_WORLD_HEIGHT = ((h/w)*WORLD_WIDTH);

        camera = new OrthographicCamera();
        this.camera.setToOrtho(false, WORLD_WIDTH, (h/w)*WORLD_WIDTH);
...
//        parameter.borderWidth = (int)(10.0f*(Gdx.graphics.getHeight()/2000.0f));;
...
//        if(preferences.getInstruction(preferences.INSTRUCTION)==1) {helper_text="swipe for game modes";} else {helper_text="tap to release";}
    ...
    public void screenTap(Vector3 mousepos, String type) {
        if(state=="GAMEOVER" || state == "COLLECTIBLES") {
            ...
            if(button_pos.sub(new Vector2(mousepos.x*(WORLD_WIDTH/Gdx.graphics.getWidth()), REAL_WORLD_HEIGHT -  (mousepos.y*(REAL_WORLD_HEIGHT/Gdx.graphics.getHeight())))).len() < (playbutton.getSize().x/2.0f* Constants.PPM)) {
    ...
    public void setScore(int score) {score_layout.setText(score_font, String.valueOf(score), new Color(1.0f, 1.0f, 1.0f, score_opacity), score_layout.width, 1, false); this.score = score;}
    ...
    public void update(float dt) {
        ...
        //CHANGE CAMERA
        if(state == "PLAY") {
            if((entities.getPlayer().getPosition().y * Constants.PPM)<entities.getLastDeathHeight()-(3.5f*REAL_WORLD_HEIGHT) && !instruction_given) {
            ...
            if(entities.getGameMode() == "FALL") {
                if((((entities.getPlayer().getPosition().y * Constants.PPM)+(WORLD_WIDTH*1.25f)) < cameraypos + (camera_velocity * Constants.PPM * dt))) {
                    cameraypos = ((entities.getPlayer().getPosition().y * Constants.PPM)+(WORLD_WIDTH*1.25f));
                    camera.position.set(camera.position.x, ((entities.getPlayer().getPosition().y * Constants.PPM)+(WORLD_WIDTH/4.0f)),
                            camera.position.z);
                } else if (((entities.getPlayer().getPosition().y * Constants.PPM)+(WORLD_WIDTH/4.0f)) < cameraypos + (camera_velocity * Constants.PPM * dt)) {
                    cameraypos += (camera_velocity * Constants.PPM * dt);
                    camera.position.set(camera.position.x, ((entities.getPlayer().getPosition().y * Constants.PPM)+(WORLD_WIDTH/4.0f)),
                            camera.position.z);
                } else {
                    cameraypos += (camera_velocity * Constants.PPM * dt);
                ...
                camera.position.set(camera.position.x, (entities.getPlayer().getPosition().y * Constants.PPM),
                        camera.position.z);
                cameraypos = (entities.getPlayer().getPosition().y * Constants.PPM);
            }
            System.out.println(camera_velocity);
            camera.update();
            if(camera_velocity>-15f) {
                camera_velocity += camera_acceleration * Constants.PPM * dt;
            }
            if (((entities.getPlayer().getPosition().y * Constants.PPM) > camera.position.y + (camera_velocity * Constants.PPM * dt)
                    + (REAL_WORLD_HEIGHT / 2))) {
        ...
        if(state=="SCROLL") {
            ...
            if((scrollvel>0 && camera.position.y>scrolllimit) || (scrollvel<0 && camera.position.y<scrolllimit)) {
                ...
                if(scrollchangeto=="STARTSCREEN") {
                    ...
                    last_step = 0;
                    this.blue = bg_colors.get(last_step);
                    color_speed = new Vector3(prev_color.r-blue.r, prev_color.g-blue.g, prev_color.b-blue.b).scl(1.0f/color_timeperiod);
                    color_current_time = 0.0f;
                    flash = true;
                    entities.play(world);
                }
                ...
                camera.position.set(camera.position.x, camera.position.y + (camera_velocity * Constants.PPM * dt),
                        camera.position.z);
                cameraypos = camera.position.y + (camera_velocity * Constants.PPM * dt);
                camera.update();
            }
        }
...
//        }
        ...
        if(flash) {
            current_color = new Color(current_color.r + (dt*color_speed.x), current_color.g + (dt*color_speed.y), current_color.b + (dt*color_speed.z), 1.0f);
            color_current_time+=dt;
            if(color_current_time>color_timeperiod) {
                color_current_time=0.0f;
                if (current_color.r - 0.01f <= blue.r && current_color.r <= blue.r + 0.01f &&
                        current_color.g - 0.01f <= blue.g && current_color.g <= blue.g + 0.01f &&
                        current_color.b - 0.01f <= blue.b && current_color.b <= blue.b + 0.01f) {
                    flash = false;
                } else {
                    color_speed = new Vector3(blue.r-current_color.r, blue.g-current_color.g, blue.b-current_color.b).scl(1.0f/color_timeperiod);
                }
            }
        }
...
//      if(preferences.getInstruction(preferences.INSTRUCTION)==1) {helper_text="swipe for game modes";} else {helper_text="tap to release";}
    ...
    public boolean isLoaded() {
        if(th.isFinishedLoading()) {
            ...
            playbutton = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createTextBox(new Vector2(WORLD_WIDTH/2.0f/ Constants.PPM, 0.0f), 7.0f, 7.0f, world), th.getTexture("play.png"),
                    new Vector2(14.0f, 14.0f), "emoji");
            title = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createTextBox(new Vector2((WORLD_WIDTH*0.5f/ Constants.PPM), REAL_WORLD_HEIGHT*0.65f/ Constants.PPM), WORLD_WIDTH*0.8f/ Constants.PPM/2.0f, (WORLD_WIDTH*0.8f)/2.0f*(170.0f/847.0f)/ Constants.PPM, world), th.getTexture("title.png"),
                    new Vector2(WORLD_WIDTH*0.8f/ Constants.PPM, (WORLD_WIDTH*0.8f)*(170.0f/847.0f)/ Constants.PPM), "emoji");

            balltexture = th.getTexture("score.png");

            helper = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createCollisionBody(new Vector2(helpersize, DEVICE_HEIGHT/ Constants.PPM*0.75f),
            ...
            chaser = new com.pendulum.game.objects.Entity(com.pendulum.game.objects.ObjectHandler.createTextBox(new Vector2(0.0f, DEVICE_HEIGHT/ Constants.PPM*2.0f), WORLD_WIDTH/2.0f/ Constants.PPM, REAL_WORLD_HEIGHT/60.0f/ Constants.PPM, world), th.getTexture("chaser1.png"),
                    new Vector2(WORLD_WIDTH/ Constants.PPM, REAL_WORLD_HEIGHT/30.0f/ Constants.PPM), "chaser");
    ...
    public boolean tapOnScreenButton(Vector3 mousepos, String type) {
        float size = 5f;
        Vector2 sound_pos = new Vector2(size*0.5f* Constants.PPM, size*0.5f* Constants.PPM);

        if(state!="TITLESCREEN" && state!="FADE_IN" && state!="FADE_IN_GAME") {
             if (sound_pos.sub(new Vector2(mousepos.x * (WORLD_WIDTH / Gdx.graphics.getWidth()), REAL_WORLD_HEIGHT - (mousepos.y * (REAL_WORLD_HEIGHT / Gdx.graphics.getHeight())))).len() < (size / 2.0f * Constants.PPM)) {
                if (type == "TOUCH_UP") {
    ...
    public void render() {
        ...
        if(!loading) {
            app.overlay.begin();
            app.overlay.setShader(shaderHandler.getBlurShader());
            shaderHandler.getBlurShader().setUniformf("Color", current_color.r, current_color.g, current_color.b, current_color.a);
            shaderHandler.getBlurShader().setUniformi("tex", -1);
            ...
            if(!playing) {
                ...
                title.getBody().setTransform(title.getBody().getPosition().x, REAL_WORLD_HEIGHT * 0.65f / Constants.PPM + (entities.getAngle() * 1.5f), title.getBody().getAngle());
            }
            ...
            if(state!="TITLESCREEN" && state!="FADE_IN_GAME") {
                ...
                app.overlay.draw(balltexture, (DEVICE_WIDTH * 11.0f / 16.0f), (DEVICE_HEIGHT * 0.95f) - (pellet_layout.height / 2) - (Constants.SCORE_RADIUS * Constants.PPM / 2), Constants.SCORE_RADIUS * Constants.PPM, Constants.SCORE_RADIUS * Constants.PPM);
                font.draw(app.overlay, pellet_layout, (DEVICE_WIDTH * 11.0f / 16.0f) + (Constants.SCORE_RADIUS * 2 * Constants.PPM), (DEVICE_HEIGHT * 0.95f));
            ...
            if(state=="FADE_IN") {
                ...
                app.overlay.setColor(c.r, c.g, c.b, fadein_transparency);
                app.overlay.draw(background, 0.0f, 0.0f, DEVICE_WIDTH, DEVICE_HEIGHT);
